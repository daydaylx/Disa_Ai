    ...
    <div className={cn("p-3", className)}>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-muted-foreground">
        <div>
          ...
          {model.context ? `${model.context.toLocaleString()} Tokens` : "k. A."}
        </div>
        <div>
          <span className="font-medium">Provider:</span> {model.provider ?? "k. A."}
        </div>
        <div>
          <span className="font-medium">Prompt-Kosten:</span> {fmtUSD(model.pricing?.prompt)}
        </div>
        <div>
          <span className="font-medium">Completion-Kosten:</span>{" "}
          {fmtUSD(model.pricing?.completion)}
        </div>
      </div>
      {model.tags?.length ? (
