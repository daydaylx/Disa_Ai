...
- **CI/CD**: Das Repository ist auf Cloudflare Pages optimiert (`deploy/cloudflare/cloudflare-pages.json` + `deploy/workers/wrangler.toml`). Deployments laufen über `npm run build`, danach werden die Artefakte direkt auf Pages veröffentlicht; `public/_headers` und `_routes.json` werden automatisch übernommen.
