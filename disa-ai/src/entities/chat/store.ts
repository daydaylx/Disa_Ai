...
type ChatState = {
  ...
  addMessage: (chatId: string, msg: Omit<ChatMessage, "id" | "ts"> & Partial<Pick<ChatMessage, "id" | "ts">>) => ChatMessage;
  clearChat: (id: string) => void;
  updateChatMeta: (id: string, meta: Partial<Pick<Chat, "modelId" | "personaId" | "archived" | "summary">>) => void;
...
export const useChatStore = create<ChatState>()(
  persist(
    (set, get) => ({
      ...
      createChat: (init) => {
        const chat: Chat = { ...initialChat(), ...init, id: uuid(), createdAt: Date.now(), updatedAt: Date.now() };
      ...
      duplicateChat: (id) => {
        ...
        const dupId = uuid();
        const dup: Chat = { ...src, id: dupId, title: src.title + " (Kopie)", createdAt: Date.now(), updatedAt: Date.now() };
        set((s) => ({
          chats: [dup, ...s.chats],
      ...
      deleteChat: (id) =>
        set((s) => {
          ...
          return { chats: rest.length ? rest : [initialChat()], messages: msgs, currentChatId: nextId };
        }),
      ...
      addMessage: (chatId, msg) => {
        ...
        set((s) => {
          ...
          if (chat) {
            const shouldName = chat.title === "Neuer Chat" && msg.role === "user" && !list.some((m) => m.role === "user");
            const newTitle = shouldName ? (msg.content || "Neuer Chat").slice(0, 48) : chat.title;
            chats = chats.map((c) => (c.id === chatId ? { ...c, title: newTitle, updatedAt: Date.now() } : c));
          }
          return { chats, messages: { ...s.messages, [chatId]: [...list, message] } };
        });
        return message;
      },
      ...
      updateChatMeta: (id, meta) =>
        set((s) => ({ chats: s.chats.map((c) => (c.id === id ? { ...c, ...meta, updatedAt: Date.now() } : c)) })),
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        if (s && !s.currentChatId && s.chats[0]) s.currentChatId = s.chats[0].id;
      }
    }
  )
);
