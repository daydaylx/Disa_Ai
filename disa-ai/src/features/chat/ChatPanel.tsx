...
import type { ChatMessage as ORMessage } from "@/lib/types";
...
export default function ChatPanel({ client, modelId, apiKeyPresent, onOpenSettings, systemPrompt, onOpenChats }: Props) {
  ...
  const { offset, active } = usePullToRefresh(listRef, async () => {
    await new Promise((r) => setTimeout(r, 350)); // Platzhalter für z.B. Reconnect
  });

  useEdgeSwipe(onOpenChats ? { enabled: true, edge: "left", onOpen: () => onOpenChats() } : { enabled: false });
  ...
  async function send() {
    ...
    setBusy(true);
    try {
      const historyMsgs: ORMessage[] = messages.map(({ role, content }) => ({ role, content }));
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
        ...
        {disabledReason && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
            <div className="flex items-center justify-between border-b border-destructive/20 bg-destructive/10 px-4 py-3">
              ...
              <button onClick={onOpenSettings} className="rounded-lg bg-destructive/20 px-3 py-1 text-xs font-medium text-destructive transition-colors hover:bg-destructive/30">
                Einstellungen
              </button>
            </div>
          </motion.div>
      ...
      <div ref={listRef} className="scroll-smooth overscroll-y-contain -webkit-overflow-scrolling-touch flex-1 min-h-0 overflow-y-auto px-4 py-6 pb-28 md:py-8 md:pb-32">
        {/* Pull-to-Refresh Indicator */}
        <div
          style={{ transform: `translateY(${Math.max(0, offset - 60)}px)`, opacity: active ? Math.min(1, offset / 60) : 0 }}
          className="pointer-events-none mx-auto mb-2 flex w-full max-w-3xl justify-center transition-transform"
        >
          <div className="rounded-full border border-border/50 bg-secondary/60 px-3 py-1 text-xs text-muted-foreground">Aktualisieren…</div>
        </div>
          ...
          <AnimatePresence mode="popLayout">
            {renderItems.map((item, index) => {
              ...
              return (
                ...
                >
                  <div className={clsx("max-w-[85%] md:max-w-[70%] rounded-2xl px-4 py-3", item.role === "user" ? "ml-8 bubble-user" : "mr-8 bubble-ai")}>
                    <div className="space-y-3">
                      {segs.map((s, i) =>
                        s.type === "code" ? (
                          <CodeBlock key={i} code={s.value} lang={s.lang} />
                        ) : (
                          <div key={i} className="whitespace-pre-wrap break-words text-sm md:text-base">{s.value}</div>
                        )
                      )}
                    </div>
      ...
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="sticky bottom-0 z-20 border-t border-border/50 bg-background/70 px-4 py-3 backdrop-blur-xl">
        <div className="mx-auto flex max-w-3xl gap-3">
          ...
          <motion.button
            ...
            className={clsx(
              "md-ripple flex h-12 w-12 items-center justify-center rounded-xl transition-all",
