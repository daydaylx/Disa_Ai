...
${violation.nodes.map(node => node.failureSummary ? `    ${colors.dim}${node.failureSummary}${colors.reset}` : '').filter(Boolean).join('\n')}
`;
}
  ...
  try {
    ...
    for (const result of results) {
      ...
      if (result.incomplete.length > 0) {
        console.log(`${colors.yellow}⚠️  Incomplete:${colors.reset} ${result.incomplete.length} (manual review needed)`);
      }
