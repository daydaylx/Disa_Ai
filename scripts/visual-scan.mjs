...
(async () => {
  ...
  if (!ok) {
    console.error('[scan] navigation failed:', lastErr?.message || lastErr);
    await browser.close();
    process.exit(2);
  }

  // Fonts/Layout abwarten (falls unterstützt)
  try { await page.evaluate(() => (document.fonts && document.fonts.ready) || null); } catch {}

  // Animationen einfrieren → stabilere Screens
  try {
