/**
 * OpenRouter Browser-Client (Streaming, SSE)
 * - Liest API-Key aus localStorage ("disa_api_key")
 * - Liest Modell aus localStorage ("disa_model") – Settings steuern das
...
function getModel(fallback?: string) {
  try {
    return localStorage.getItem(MODEL_KEY) || fallback || "meta-llama/llama-3.3-70b-instruct:free";
  } catch {
    return fallback || "meta-llama/llama-3.3-70b-instruct:free";
  }
}
