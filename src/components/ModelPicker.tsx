...
type Props = {
  ...
  allow?: string[] | null
  /** Wenn gesetzt, wird der Policy-Filter darauf synchronisiert (z. B. aus Rolle). */
  policyFromRole?: Safety | "any"
}

type SortKey = "alpha" | "price" | "context"

export default function ModelPicker({ value, onChange, allow = null, policyFromRole }: Props) {
  ...
  // Rolle → Policy auf Filter spiegeln, aber nicht aggressiv überschreiben, wenn der Nutzer bereits etwas gewählt hat
  React.useEffect(() => {
    if (!policyFromRole) return
    if (policy === "any" && policyFromRole !== "any") setPolicy(policyFromRole)
  }, [policyFromRole]) // eslint-disable-line react-hooks/exhaustive-deps

  const providers = React.useMemo(() => listProviders(all), [all])

  function toggleProv(p: string) {
    setProvSel((cur) => cur.includes(p) ? cur.filter(x => x !== p) : [...cur, p])
  }
      ...
      <div className="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white/80 dark:bg-neutral-900/70 backdrop-blur p-3">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 items-end">
          ...
          <div className="lg:col-span-4 flex items-center gap-4">
            ...
            <button type="button" onClick={() => { setQ(""); setProvSel([]); setFreeOnly(false); setVisionOnly(false); setMinCtx(0); setMaxPrice1k(null); setSort("alpha"); /* Policy bleibt bewusst */ }} className="ml-auto text-sm px-3 py-1.5 rounded-xl border border-neutral-300 dark:border-neutral-700 hover:bg-neutral-100 dark:hover:bg-neutral-800">
              Reset
            </button>
          </div>
        </div>
