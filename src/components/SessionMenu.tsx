...
Dropdown-Panel (Suchen, Liste, Neu, Umbenennen, Duplizieren, Löschen)

Sauberes ClickAway, z-Index, Keyboard-Handling
*/
...
// ClickAway: Panel nur schließen, wenn außerhalb geklickt wird (kein brutales Blur beim Edit)
React.useEffect(() => {
function onDocClick(e: MouseEvent) {
const root = rootRef.current;
if (!root) return;
const target = e.target as Node | null;
if (target && root.contains(target)) return; // Klick innerhalb -> nicht schließen
setOpen(false);
}
...
// Panel öffnen -> Suchfeld fokusieren
React.useEffect(() => {
if (open) {
...
if (name.length > 0) renameConversation(editingId, name);
setEditingId(null);
setEditValue("");
}
...
setQuery("");
// Menü offen lassen, damit man sofort umbenennen kann – UX-Entscheidung:
// Wer das direkt schließen will, hier alternativ: setOpen(false);
}
...
// ENTER wählt erste gefilterte Konversation
...
Chats
  ...
  {open && (
    <div
      ...
      className="absolute right-0 mt-2 w-80 max-w-[calc(100vw-1rem)] max-h-[70vh] overflow-hidden rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 shadow-xl z-50"
    >
