...
export function ChatHistorySidebar(props: ChatHistorySidebarProps) {
  ...
  if (isMobile) {
    return <ChatHistoryBottomSheet {...props} />;
  }

  return <ChatHistorySidePanel {...props} />;
}
    ...
    <div className="flex h-full flex-col overflow-hidden">
      <div className="flex flex-col gap-4 px-[clamp(12px,4vw,20px)] pt-4 md:px-6">
        <div className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="icon" onClick={onClose} aria-label="Chat-Verlauf schließen">
            <X className="h-5 w-5" />
          </Button>
        </div>
                  ...
                  <li key={conversation.id} role="presentation">
                    <div
                      className={cn(
                        ...
                        isActive && "border-[var(--acc1)] shadow-[var(--shadow-glow-brand-subtle)]",
                      )}
...
function ChatHistorySidePanel(props: ChatHistorySidebarProps) {
...
function ChatHistoryBottomSheet(props: ChatHistorySidebarProps) {
  return (
    <BottomSheet isOpen={props.isOpen} onClose={props.onClose} title="Chat-Verlauf">
      <ChatHistoryView {...props} />
    </BottomSheet>
  );
}
