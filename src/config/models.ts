...
function detectSafety(provider?: string, id?: string, name?: string): Safety {
  ...
  const LOOSE_PROVIDERS = ["meta", "meta-llama", "qwen", "deepseek", "tiiuae", "nousresearch", "phind", "gryphe", "sao10k", "agentica-org"]
  ...
  if (/\buncensored\b|\bnsfw\b|\bno[-_ ]safety\b/.test(s)) return "loose"
...
function normalizeEntry(raw: any): ModelEntry | null {
  ...
  entry.safety = detectSafety(provider, id, raw?.name)
  return entry
}
...
export function hasTag(m: ModelEntry, tag: string): boolean { return m.tags.includes(tag) }
