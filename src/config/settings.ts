...
export function getStyle(): StyleKey {
  try {
    const raw = localStorage.getItem(STYLE_KEY) as StyleKey | null;
    return raw ?? "blunt_de";
  } catch {
    return "blunt_de";
  }
}
export function setStyle(k: StyleKey) {
  try {
...
export function setTemplateId(id: string | null) {
  try {
...
export function getUseRoleStyle(): boolean {
  try {
    const raw = localStorage.getItem(ROLESTYLE_KEY);
    return raw === null ? true : raw === "1" || raw === "true";
  } catch {
    return true;
  }
}
export function setUseRoleStyle(on: boolean) {
  try {
    localStorage.setItem(ROLESTYLE_KEY, on ? "1" : "0");
  } catch {}
}
