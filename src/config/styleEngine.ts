...
function roleOverlay(roleId: string | null): Overlay {
  ...
  if (roleId === "fitness_nutrition_coach")
    return {
      formality: "medium",
      humor: "none",
      bulletPreference: "prefer",
  ...
  if (roleId === "erotic_creative_author")
    return {
      formality: "medium",
      humor: "none",
      bulletPreference: "allow",
...
function profileToText(p: StyleProfile): string {
  const lines: string[] = [];
  lines.push(
    `Feintuning: Ton=${p.formality}, Kürze=${p.concision}, Humor=${p.humor}, Emojis=${p.emojis ? "ja" : "nein"}.`,
  );
  if (p.priority.length) lines.push(`Priorität: ${p.priority.join(" → ")}.`);
  if (p.structure.length) lines.push(`Struktur: ${p.structure.join(" → ")}.`);
  lines.push(
    `Listen: ${p.bulletPreference}${p.maxBullets > 0 ? ` (max. ${p.maxBullets} Punkte)` : ""}.`,
  );
...
export function generateRoleStyleText(
  roleId: string | null,
  style: StyleKey,
