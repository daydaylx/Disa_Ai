...
function sortByUpdated(a: ConversationMeta, b: ConversationMeta): number {
...
function genId(): string {
  return globalThis.crypto?.randomUUID?.() ?? `m_${Math.random().toString(36).slice(2)}`;
}
...
function writeIndex(ids: string[]): void {
  setJSON(convIndexKey(), ids);
}
function readMeta(id: string): ConversationMeta | null {
  return getJSON<ConversationMeta>(convMetaKey(id));
}
function writeMeta(meta: ConversationMeta): void {
  setJSON(convMetaKey(meta.id), meta);
}
function readMsgs(id: string): ChatMessage[] {
  const arr = getJSON<ChatMessage[]>(convMsgsKey(id));
  return Array.isArray(arr) ? arr : [];
}
function writeMsgs(id: string, messages: ChatMessage[]): void {
  setJSON(convMsgsKey(id), messages);
}
...
export function useConversations(): UseConversations {
  ...
  const reload = useCallback(() => {
    ...
    setItems(metas);
    const nextActive =
      activeId && metas.find((m) => m.id === activeId) ? activeId : (metas[0]?.id ?? null);
  ...
  const createConversation = useCallback((title?: string) => {
    ...
    const meta: ConversationMeta = {
      id,
      title: title?.trim() || "Neue Unterhaltung",
      createdAt: now(),
      updatedAt: now(),
    };
    const index = readIndex();
  ...
  const addMessage = useCallback(
    (msg: Omit<ChatMessage, "id" | "ts"> & { id?: string; ts?: number }) => {
      if (!activeId) return;
      ...
      const full: ChatMessage = {
        ...
        content: msg.content,
        ts,
        ...((msg as any).meta ? { meta: (msg as any).meta } : {}),
      };

      const list = readMsgs(activeId);
      const nextList = [...list, full];
      writeMsgs(activeId, nextList);
  ...
  const append = useCallback(
    (id: string, msg: Omit<ChatMessage, "id" | "ts"> & { id?: string; ts?: number }) => {
      const ts = typeof msg.ts === "number" ? msg.ts : now();
