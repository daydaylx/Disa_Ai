...
const FALLBACK_STYLES: StyleItem[] = [
  { id: "neutral", name: "Neutral Standard", system: "Du bist ein sachlicher, hilfreicher Assistent. Antworte klar und strukturiert.", description: "Nüchtern, faktisch, ohne Floskeln." },
  { id: "sarkastisch", name: "Sarkastisch Direkt", system: "Du bist direkt mit trockenem Humor. Ironisch, aber respektvoll.", description: "Bissig, aber hilfreich." }
];
...
export function normalizeStyles(input: unknown): StyleItem[] {
  const arr = Array.isArray((input as any)?.styles) ? (input as any).styles
            : Array.isArray(input) ? input
            : [];

  const out: StyleItem[] = arr.map((raw: any, i: number) => {
    const r: any = raw || {};
    ...
    const allow = Array.isArray(r.allow) ? r.allow.filter((x: unknown) => typeof x === "string") : undefined;
...
export async function loadStyles(signal?: AbortSignal): Promise<StyleItem[]> {
  const CANDIDATE_URLS = [
    "/persona.yaml", "/persona.yml",
    "/styles.yaml", "/styles.yml",
    "/persona.json", "/personas.json",
    "/styles.json", "/style.json"
  ];
