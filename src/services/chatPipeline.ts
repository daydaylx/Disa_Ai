...
export function buildSystemFromSettings(opts: {
  nsfw: boolean;
  style: StyleKey;
  ...
  const roleStyle = generateRoleStyleText(
    opts.roleTemplateId ?? null,
    opts.style,
    opts.useRoleStyle ?? true,
  );
...
export function buildMessages(args: {
  ...
  if (Array.isArray(args.history) && args.history.length > 0)
    out.push(...args.history.filter((m) => m.role !== "system"));
