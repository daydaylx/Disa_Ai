...
export function PullToRefresh({
  ...
  const handleTouchMove = useCallback(
    (e: TouchEvent) => {
      if (disabled || status === "refreshing" || !isAtTop()) return;

      const currentY = e.touches[0]?.clientY ?? 0;
      const deltaY = currentY - startY.current;

      // Nur pull-down erlauben
      if (deltaY > 0) {
        // Prevent default scroll
        e.preventDefault();

        // Rubber-band effect (langsamer bei größerer Distanz)
        const rubberBand = Math.min(deltaY * 0.5, maxPullDistance);
    ...
    [disabled, status, threshold, maxPullDistance, isAtTop],
  );
