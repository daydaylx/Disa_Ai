...
(function(){
  ...
  function ensureToaster(): HTMLDivElement {
    ...
    if (!el) {
      ...
      Object.assign(el.style,{
        position:"fixed",zIndex:"9999",top:"12px",left:"50%",transform:"translateX(-50%)",
        padding:"8px 12px",borderRadius:"10px",fontSize:"12px",
        boxShadow:"0 4px 12px rgba(0,0,0,.35)",pointerEvents:"none",display:"none",
        color:"#0b1220",background:"rgba(78,133,255,.95)" // default brand
  ...
  function colorFor(intent: Intent){
    switch (intent) {
      case "success": return "rgba(16,185,129,.95)";   // green
      case "warning": return "rgba(245,158,11,.95)";   // amber
      case "error":   return "rgba(239,68,68,.95)";    // red
      default:        return "rgba(78,133,255,.95)";   // brand
    }
  }
  ...
  function showToast(message: string, intent: Intent="success", ms=3000){
    ...
    hideTimer = window.setTimeout(()=>{ el.style.display="none"; }, ms);
  }
  ...
  try{
    ...
    if (s.has("nuke-sw") || s.has("no-sw")) {
      ...
      u.searchParams.delete("nuke-sw"); u.searchParams.delete("no-sw");
